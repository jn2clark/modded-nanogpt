...
def apply_aurora_relax(aurora: Tensor, plain: Tensor) -> Tensor:
    ...
    mixed = aurora.float() + AURORA_RELAX_LAMBDA * residual * clip_scale
...
def scale_radial_update(update: Tensor, param: Tensor, step: int, eps: float = 1e-12) -> Tensor:
    ...
    radial_scale = torch.where(
        coeff < 0,
        update_f.new_tensor(radial_outward_scale_for_step(step)),
