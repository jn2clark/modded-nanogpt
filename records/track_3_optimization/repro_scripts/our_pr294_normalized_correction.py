...
SEED = args.seed
# Hardcoded final schedule constants. TRAIN_STEPS is only an override for
# longer-horizon/generalization sweeps; the default remains the official PR run.
...
FINAL_MUON_WD = 0.025
EXPERIMENT_NAME = "radial-dampen-plus-normalized-correction"
EXPERIMENT_INTUITION = "PR294 radial damping plus PR295-style normalized correction before Muon Newton-Schulz."
...
NORMALIZED_CORRECTION_POWER = float(os.environ.get("NORMALIZED_CORRECTION_POWER", "0.5"))
...
def apply_normalized_correction(G: Tensor, eps: float = 1e-12) -> Tensor:
    if NORMALIZED_CORRECTION_POWER <= 0.0:
        return G
    X = G.float()
    row_norm = X.norm(dim=-1, keepdim=True).clamp_min(eps)
    col_norm = X.norm(dim=-2, keepdim=True).clamp_min(eps)
    scale = (row_norm * col_norm).pow(-NORMALIZED_CORRECTION_POWER)
    corrected = X * scale
    corrected = corrected * (gram_frobenius_norm_estimate(X, keepdim=True) / gram_frobenius_norm_estimate(corrected, keepdim=True)).to(corrected.dtype)
    return corrected.to(G.dtype)
...
def muon_update(update, second_moment, step, beta2=NOR_BETA2, use_contra=True, use_soft=True):
    ...
    corrected_update = apply_normalized_correction(update)
    ns_update = zeropower_via_newtonschulz5(corrected_update)
    ...
    if use_soft:
        soft_update = soft_via_newtonschulz5(corrected_update, SOFT_MUON_P, SOFT_MUON_SCALE, SOFT_MUON_INPUT_NORM)
...
print0(f"Using normalized_correction_power={NORMALIZED_CORRECTION_POWER}")
...
extra_val_steps = set(range(2850, 3021, 25))
