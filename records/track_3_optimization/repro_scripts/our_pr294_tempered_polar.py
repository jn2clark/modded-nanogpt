...
FINAL_MUON_WD = 0.025
EXPERIMENT_NAME = "radial-dampen-tempered-polar"
EXPERIMENT_INTUITION = "PR294 radial damping plus a small clipped pre-polar residual blended into the Newton-Schulz polar output."
...
def apply_tempered_polar(prepolar: Tensor, polar: Tensor, polar_norm: Tensor, step: int) -> Tensor:
    ...
    clip_scale = (max_residual_norm / residual_norm).clamp(max=1.0)
    candidate = polar.float() + rho * residual * clip_scale.to(residual.dtype)
    candidate_norm = gram_frobenius_norm_estimate(candidate)
    candidate = candidate * (polar_norm / candidate_norm).to(candidate.dtype)
    return candidate.to(polar.dtype)
...
extra_val_steps = set(range(DENSE_EVAL_START, DENSE_EVAL_END + 1, DENSE_EVAL_STRIDE)) | {2980, 2985, 2990, 2995}
