"""
our_pr294_history_tempered_polar.py
...
stack Muon-history with tempered polar Muon
...
SEED = args.seed
# TRAIN_STEPS and SCHEDULE_STEPS are overrides for validation sweeps. Defaults
# keep the best local PR294 history-taper script unchanged except for the
# conservative tempered-polar residual blend.
FINAL_TRAIN_STEPS = int(os.environ.get("TRAIN_STEPS", "3020"))
FINAL_SCHEDULE_STEPS = int(os.environ.get("SCHEDULE_STEPS", "3105"))
FINAL_LR_POWER = 1.2
...
FINAL_MUON_WD = 0.025
EXPERIMENT_NAME = "radial-dampen-history-tempered-polar"
EXPERIMENT_INTUITION = "PR294 radial damping plus Muon pre-polar history extrapolation, then a conservative clipped residual blend after Newton-Schulz."
...
CONTRA_TO_NORMAL_END_STEP = int(os.environ.get("CONTRA_TO_NORMAL_END_STEP", "2000"))
...
HISTORY_DECAY_START_FRAC = float(os.environ.get("HISTORY_DECAY_START_FRAC", "0.39"))
HISTORY_DECAY_END_STEP = int(os.environ.get("HISTORY_DECAY_END_STEP", "0"))
DENSE_EVAL_START = int(os.environ.get("DENSE_EVAL_START", "2850"))
DENSE_EVAL_END = int(os.environ.get("DENSE_EVAL_END", "3020"))
DENSE_EVAL_STRIDE = int(os.environ.get("DENSE_EVAL_STRIDE", "25"))
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                if base_i + rank < len(params):
                    ...
                    momentum_update = apply_muon_history(momentum_update, state, self.step_count, group)
                    update = muon_update(
                        momentum_update,
                        state["second_moment"],
                        self.step_count,
...
print0(f"Using history_decay_start_frac={HISTORY_DECAY_START_FRAC}")
print0(f"Using history_decay_end_step={HISTORY_DECAY_END_STEP}")
...
muon_history = dict(
    ...
    muon_history_decay_start_step=int(HISTORY_DECAY_START_FRAC * train_steps),
    muon_history_decay_end_step=HISTORY_DECAY_END_STEP if HISTORY_DECAY_END_STEP > 0 else train_steps,
...
print0(f"Using muon_history={muon_history}")
