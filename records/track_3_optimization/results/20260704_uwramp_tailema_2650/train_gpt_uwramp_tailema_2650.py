...
UW_TAPER_START = 2350
UW_TAPER_END = 2660
DENOM_POWER_END = 0.72
TRUST_GATE_ONE_FROM_STEP = 2400
                ...
                if base_i + rank < len(params):
                    ...
                    if use_soap:
                        if p in self.attn_soap_params:
                            ...
                            if self.step_count >= TRUST_GATE_ONE_FROM_STEP and p in self.attn_proj_soap_params:
...
def set_hparams(step):
    assert step >= 0
    global SOAP_TARGET_UW, NONSOAP_TARGET_UW, SOAP_DENOM_POWER
    frac = min(1.0, max(0.0, (step - UW_TAPER_START) / max(1, UW_TAPER_END - UW_TAPER_START)))
    SOAP_TARGET_UW = NONSOAP_TARGET_UW = TARGET_UW + frac * (TARGET_UW_END - TARGET_UW)
    SOAP_DENOM_POWER = 0.50 + frac * (DENOM_POWER_END - 0.50)
