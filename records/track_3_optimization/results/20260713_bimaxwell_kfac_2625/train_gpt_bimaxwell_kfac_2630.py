"""
train_gpt_bimaxwell_kfac_2630.py

Experimental composite: PR #339 Bi-Maxwell base plus output-head KFAC.
No result is claimed until the fixed-seed validation run completes.
...
# Output-head activation-covariance preconditioner. It acts on the input
# geometry of the vocabulary projection and leaves the Bi-Maxwell hidden-matrix
# temporal kernel unchanged.
OUTPUT_KFAC_STRENGTH = 0.75
OUTPUT_KFAC_POWER = 0.5
OUTPUT_KFAC_BETA = 0.9
OUTPUT_KFAC_DAMPING = 0.05
OUTPUT_KFAC_START = 2000
OUTPUT_KFAC_END = 2640
OUTPUT_KFAC_REFRESH = 8
OUTPUT_KFAC_SUBSAMPLE = 16
...
_output_kfac_accum = None
_output_kfac_count = None
_output_kfac_cov = None
_output_kfac_inverse_power = None
_output_kfac_hook_handle = None

@torch.compiler.disable
@torch.no_grad()
def _output_kfac_hook(module, inputs, output):
    step = optimizer2.step_count
    if not model.training or not OUTPUT_KFAC_START <= step < OUTPUT_KFAC_END:
        return
    x = inputs[0].detach().flatten(0, -2)[::OUTPUT_KFAC_SUBSAMPLE].float()
    _output_kfac_accum.addmm_(x.mT, x)
    _output_kfac_count.add_(x.size(0))

@torch.no_grad()
def _enable_output_kfac(step):
    global _output_kfac_accum, _output_kfac_count, _output_kfac_hook_handle
    if _output_kfac_hook_handle is not None or step < OUTPUT_KFAC_START:
        return
    _output_kfac_accum = torch.zeros(768, 768, dtype=torch.float32, device=device)
    _output_kfac_count = torch.zeros((), dtype=torch.float32, device=device)
    _output_kfac_hook_handle = model.proj.register_forward_hook(_output_kfac_hook)
    torch.compiler.reset()

@torch.no_grad()
def _apply_output_kfac(step):
    global _output_kfac_cov, _output_kfac_inverse_power
    if not OUTPUT_KFAC_START <= step < OUTPUT_KFAC_END:
        return
    if step == OUTPUT_KFAC_START:
        assert _output_kfac_count.item() > 0, "output KFAC hook did not capture activations"

    dist.all_reduce(_output_kfac_accum, op=dist.ReduceOp.SUM)
    dist.all_reduce(_output_kfac_count, op=dist.ReduceOp.SUM)
    batch_cov = _output_kfac_accum / _output_kfac_count.clamp_min(1.0)
    if _output_kfac_cov is None:
        _output_kfac_cov = batch_cov.clone()
    else:
        _output_kfac_cov.lerp_(batch_cov, 1.0 - OUTPUT_KFAC_BETA)
    _output_kfac_accum.zero_()
    _output_kfac_count.zero_()

    if _output_kfac_inverse_power is None or (step - OUTPUT_KFAC_START) % OUTPUT_KFAC_REFRESH == 0:
        eigenvalues, eigenvectors = torch.linalg.eigh(_output_kfac_cov)
        damping = OUTPUT_KFAC_DAMPING * eigenvalues.mean().clamp_min(1e-8)
        scales = (eigenvalues.clamp_min(0.0) + damping).pow(-OUTPUT_KFAC_POWER)
        _output_kfac_inverse_power = (eigenvectors * scales.unsqueeze(0)) @ eigenvectors.mT

    grad = model.proj.weight.grad
    raw = grad.float()
    natural = raw @ _output_kfac_inverse_power
    raw_norm = raw.norm()
    natural.mul_(raw_norm / natural.norm().clamp_min(1e-12))
    mixed = raw.lerp(natural, OUTPUT_KFAC_STRENGTH)
    mixed.mul_(raw_norm / mixed.norm().clamp_min(1e-12))
    grad.copy_(mixed.to(grad.dtype))
